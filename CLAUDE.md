...
| # | 任務 | 說明 | 狀態 |
|---|------|------|------|
| G1 | Demo 版基礎遊戲 | 5 種任務、多步驟選擇、獎金系統、結算 | ✅ v1.0 |
| G2 | 更多情境模板 | 每種任務擴充至 15-20 組模板 | 🟡 |
| G3 | 技能樹 / 升級系統 | 加快處理速度、同時處理多任務、解鎖新機台 | 🔴 |
| G4 | 成就系統 | 「連續完成 10 個任務」「零失敗通關」等 | 🔴 |
| G5 | 事件系統擴充 | 突發停機、主管巡廠、稽核、消防演習等 | 🔴 |
| G6 | 人物對話 / 劇情片段 | 同事互動、八卦、午餐選擇等生活元素 | 🔴 |
| G7 | 音效系統 | 新任務提示音、完成/失敗音效、BGM | 🔴 |
| G8 | **3D FAB 行走模式** | **長期願景**：加入 FAB 廠房 3D 場景，玩家以第一人稱或第三人稱在廠房中行走，走到機台前觸發任務。技術方案：Three.js 或 Babylon.js。需要 FAB 廠房模型（走道、Bay、機台群）。這是從文字選擇遊戲進化為沉浸式 3D 遊戲的關鍵里程碑。 | 🔵 長期 |
| G9 | 多個機台區域切換 | CVD Bay / Etch Bay / Litho Bay 等不同區域 | 🔴 |
| G10 | 排行榜（線上） | 需要後端支援，記錄全球玩家排名 | 🔵 長期 |
